--[[-----------------------------------------------------------------------------
this section based on: EditBox Widget
-------------------------------------------------------------------------------]]
...
local methods = {
        ...
        ["SetLabel"] = 
          function(self, text)
            ...
            self.alignoffset = 12
            local Fonts = ns.RP_Fonts.Fonts;
            local file;
            text = text or "Morpheus";
            print(text, Fonts[text]);
            if   text and Fonts[text] and Fonts[text]:GetItems("file")
            then for fileName, fileData in pairs(Fonts[text]:GetItems()) do if not fileData:HasFlag("missing") then file = fileName; break; end; end; end;
