...
local RPTAGS = RPTAGS;
local Module = RPTAGS.queue:NewModule("RP UnitFrames", "rpClient");

Module:WaitUntil("LOAD_UTILS_FRAMES",  function(self, ...) RPTAGS.utils.rpuf.tags.registerall(...)  end);
Module:WaitUntil("LOAD_UTILS_FRAMES", function(self, ...) RPTAGS.utils.rpuf.frames.refresh(...)    end);
Module:WaitUntil("LOAD_UTILS_FRAMES",   function(self, ...) RPTAGS.utils.rpuf.frames.refreshAll(...) end);

Module:WaitUntil("LOAD_CONST",
  function(self, event, ...)
    local RPTAGS = RPTAGS;
    RPTAGS.CONST = RPTAGS.CONST or {};
    RPTAGS.CONST.APP_COLOR_RPUF = "|cffdd9933";
  end);

Module:WaitUntil("LOAD_KEYBIND",
  function(self, event ...)
    local RPTAGS = RPTAGS;
