...
local format_spaces = {
	...
	['#rrggbbaa'] = 'rgb', ['#rrggbb'] = 'rgb',
        -- added by Oraibi-MoonGuard:
        ['|cffrrggbb'] = 'rgb', ['|caarrggbb'] = 'rgb',
        -- end of added by Oraibi-MoonGuard
	...
	elseif fmt == '#rgba' or fmt == '#rgb' then
		return string.format(
			fmt == '#rgba' and '#%1x%1x%1x%1x' or '#%1x%1x%1x',
				...
				round(a * 15))
        -- added by Oraibi-MoonGuard:
        elseif fmt == "|cffrrggbb" then
		return string.format('|cff%02x%02x%02x',
				round(x * 255),
				round(y * 255),
				round(z * 255))
        elseif fmt == "|caarrggbb" then
		return string.format('|c%02%02x%02x%02x',
				round(a * 255),
				round(x * 255),
				round(y * 255),
				round(z * 255))
...
color_module.clamp = clamp;
color_module.convert = convert;
color_module.parse = parse;
color_module.parse_rgba32 = parse_rgba32;
color_module.parse_argb32 = parse_argb32;
color_module.format = format;
color_module.hsl = new_with'hsl';
color_module.hsv = new_with'hsv';
color_module.rgb = new_with'rgb';
